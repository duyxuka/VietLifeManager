...
import { PhongBanDto, PhongBanInListDto, PhongBansService } from '@proxy/viet-life/catalog/phong-bans';
...
export class PhongBanComponent implements OnInit, OnDestroy {
  ...
  items: PhongBanInListDto[] = [];
  public selectedItems: PhongBanInListDto[] = [];
  ...
  loadData() {
    ...
    this.phongbanService
      ...
      .subscribe({
        next: (response: PagedResultDto<PhongBanInListDto>) => {
  ...
  deleteItemsConfirmed(ids: string[]){
    this.toggleBlockUI(true);
    this.phongbanService.deleteMultiple(ids).pipe(takeUntil(this.ngUnsubscribe)).subscribe({
      next: ()=>{
